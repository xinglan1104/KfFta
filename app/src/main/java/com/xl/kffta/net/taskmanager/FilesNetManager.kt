...
//    }
    ...
    fun getFileUrls(ids: ArrayList<Long>, callback: ResponseObjectCallback) {
        if (ids.isNullOrEmpty()) {
            callback.onError("没有选择ids")
            return
        }
        ...
        ids.forEach {
            urls.add("https://test.dynamictier.com/services2/serviceapi/web/DownloadFile?Token=${ApplicationParams.TOKEN}&id=$it")
        }
        if (urls.isNotEmpty()) {
